import ProjectData from '@/data/ProjectData.ts'

export default [
    new ProjectData("project-6", "AstroPark", "img/projects/Astropark.jpg", `
        <div class="paragraph">
            <strong>AstroPark</strong> Astro mini games!
        </div>
         <div class="paragraph center">
            <img class="pc-screenshot" src="img/projects/Astropark.jpg" alt="Astro" />
        </div>
        <div class="paragraph">
           Prepare to dive into new worlds as you explore an asteroid belt on your journey to the moon and uncover the magic of black holes in our observatory.
Strap on your spacesuit once more and blast off to AstroPark! Your destination for space exploration and learning.
        </div>
        <div class="paragraph">
            Main features :
            <ul>
                <li>Engine: Unity.</li>
                <li>Language: C#.</li>
                <li>Platform: Meta Quest 1/Meta Quest 2/Meta Quest 3.</li>
            </ul>
        </div>
    
        <div class="paragraph">
            <div class="notice">
            Available on <a href="https://www.meta.com/es-es/experiences/6431050480354813/" target="_blank">Meta Store</a>.
            </div>
        </div>
    <div class="paragraph center">
        <img class="pc-screenshot" src="img/projects/38982518_533017902341923_5263207808924262857_n.jpg" alt="Astro" />
        <img class="pc-screenshot" src="img/projects/39003252_805917724661203_7829550643353870462_n.jpg" alt="Newton" />
        <img class="pc-screenshot" src="img/projects/75488378_800660725129207_5894272252427719846_n.jpg" alt="SpaceShip" />
    </div>
        </div>`, "#3464eb", true, true),
    new ProjectData("project-1", "Grand Theft Laugh", "img/projects/GrandTheftLaugh.gif",
        `
    <div class="paragraph">
     <strong>Grand Theft Laugh</strong> Global Game Jam 2024.
    </div>
    <div class="paragraph center">
        <img src="img/projects/GrandTheftLaugh.gif">
    </div>
    
    <div class="paragraph">
        <ul>
        <li>Theme: Make me Laugh.</li>
        <li>Language: C#.</li>
        <li>Engine: Unity 2022.</li>
        <li>Platform: Windows.</li>
        <li>Other: GitHub, Git, Trello.</li></ul>
    </div>
<div class="paragraph">
        <div class="notice">
        Avaiable on <a href="https://vlyx.itch.io/grand-theft-laugh" target="_blank">itch.io</a>
        </div>
    </div>
    <div class="paragraph center">
    <img class="phone-screenshot" src="https://ggjv4.s3.us-west-1.amazonaws.com/files/styles/flexslider_full/s3/games/2024/716762/screenshot/image.png?VersionId=85f.2y4vjdylbQqP.ej6LRvIkf3E_2De&itok=TFKakB3N"
                                alt="img 1">
        <img class="pc-screenshot" src="https://ggjv4.s3.us-west-1.amazonaws.com/files/styles/sidebar_full/s3/games/2024/716762/featured/plinplin.png?VersionId=Umed6w67zqJrAbT9yAKyDexECWuVsknb&itok=3r9G9-gO"
                                alt="img 2">
       
    </div>
    `, "#23bd69", false, false),
    new ProjectData("project-3", "Farmoxel", "img/projects/farmoxel.PNG", `
    <div class="paragraph">
        <strong>Farmoxel</strong> is a thing of beauty that I am so proud of. I could write about it for hours.
       
    </div>
    <div class="paragraph center">
    video
        <iframe src="https://www.linkedin.com/embed/feed/update/urn:li:ugcPost:7097632850123866113?compact=1" 
        class="youtube" frameborder="0" allowfullscreen></iframe>
    </div>

    <div class="paragraph">
        Main features :
        <ul>
            <li>Language: C#.</li>
            <li>Engine: Unity.</li>
            <li>Platform: Windows.</li>
            <li>Other: GitHub, Git, Trello.</li>
        </ul>
    </div>

    <div class="paragraph">
        <div class="notice">
        Still in develop! Stay tuned on <a href="https://farteamdev.blogspot.com/" target="_blank">Blogspot</a>
        </div>
    </div>

    <div class="paragraph center">
        <img class="pc-screenshot" src="https://blogger.googleusercontent.com/img/a/AVvXsEhzNLWXAdOqq-Im0Xz26dhuZNBCv5hIaHy3iZAyUQTnToYMuU-nWLHeao9yyyDCJvBfjiAxU1qFko39w67M4ULGrwuOO9xHp6EBZqfYYhDjGmaOZNaCnrPf89TKIW2CvE9DG8Zby6J1EcnUhjUavBou8pFLv_3x0enIfjRD89STTunK83qHEEDqqz0nxAPh=w945-h600-p-k-no-nu" alt="Strawberries" />
        <img class="pc-screenshot" src="https://blogger.googleusercontent.com/img/a/AVvXsEim0sV6_GUxPRfY1JpkU-XKmZRVLZ5PZlpaAXSLYOacAHGP3icTyFYIyDIq84s-5tYPHZNY_e39CeH1rsM3dkPWxSpWsn34mnpZMT3acx6FS38MsUOMXJbiSbpoCh_b9OnEwrEg7Yl6btv6FgitADo0zD-XEYWmcQ4dJ8RvksS9RQgnOaqUS5X05PoP0v-e=w640-h360" alt="Drawing Overload Screenshot" />
    </div>
    `, "#383838", false),
    new ProjectData("project-4", "Roomaker", "https://img.itch.zone/aW1nLzEzNTUxNjYzLnBuZw==/315x250%23c/AqpKHb.png", `
    <div class="paragraph">
    <strong>Room Makersh</strong>
     is a chill 2D puzzle game for the GBJam 11.
    In this gameboy themed game, you're a lucky guy who constantly receives new furniture but struggle to quickly arrange it all!
    </div>
    <div class="paragraph center">
        <img src="https://franrosatti9.github.io/img/projects/RoomMakersGif.gif">
    </div>
       
    <div class="paragraph">
        Main features :
        <ul>
        <li>Theme: Space.</li>
        <li>Time: 15 days.</li>
        <li>Language: C#.</li>
        <li>Engine: Unity 2022.</li>
        <li>Platform: WebGL.</li>
        <li>Other: Git.</li>
        </ul>
    </div>

    <div class="paragraph">
        <div class="notice">
        WebGL version available on <a href="https://justneki.com/RoomMakersWeb/" target="_blank">itch.io</a>.
        Android version coming soon!
        </div>
    </div>

    <div class="paragraph center">
        <img class="pc-screenshot" src="https://img.itch.zone/aW1hZ2UvMjI3NTEwNy8xMzQ5OTIzOC5wbmc=/347x500/guAo4v.png" alt="Eugeneable Screenshot" />
        <img class="pc-screenshot" src="https://img.itch.zone/aW1hZ2UvMjI3NTEwNy8xMzUxMjY0MC5wbmc=/347x500/07%2F80q.png" alt="Eugeneable Screenshot" />
        <img class="pc-screenshot" src="https://img.itch.zone/aW1hZ2UvMjI3NTEwNy8xMzUxMjc3Ni5wbmc=/347x500/PvCBd8.png" alt="Eugeneable Screenshot" />
    </div>
    `, "#e80fb7"),
    new ProjectData("project-5", "Move & Move", "https://img.itch.zone/aW1nLzE0NTk4NTIxLnBuZw==/315x250%23c/Y9OzJk.png", `
        <div class="paragraph">
            <strong>Move & Move Remake</strong> Remake of my first project following SOLID.
        </div>
         <div class="paragraph center">
            <img src="img/projects/movemove.gif"
        </div>
        <div class="paragraph">
            Simple 2D game of an spaceship and some aliens.
            I remade my first game using what I've learnt in almost 7 years.
            I Applied <strong>SOLID</strong> principles and Pattern Designs such as Singleton, Observer, Strategy and Pooling.
        </div>
        <div class="paragraph">
            Main features :
            <ul>
                <li>Status: Done.</li>
                <li>Language: C#.</li>
                <li>Engine: Unity.</li>
                <li>Platform: WebGL / Android.</li>
            </ul>
        </div>
    
        <div class="paragraph">
            <div class="notice">
            WebGL build available on <a href="https://vlyx.itch.io/movemove-remake" target="_blank">itch.io</a>.
            Source code available on <a href="https://github.com/Ulysses-Alv/Move-Move-Remake" target="_blank">GitHub</a>.
            </div>
        </div>
    
        <div class="paragraph center">
        </div>`, "#e48246", false, false),
new ProjectData("project-7", "Wonderland", "img/projects/wonderland.jpg", `
        <div class="paragraph">
            <strong>Wonderland</strong> a place where dreams come true.
        </div>
         <div class="paragraph center">
             <img src="img/projects/wonderland2.jpg"
        </div>
        <div class="paragraph">
            Game made in Horizon World.
            </div>
        <div class="paragraph">
            Main features :
            <ul>
                <li>Status: Done.</li>
                <li>Language: TypeScript.</li>
                <li>Engine: Horizon World.</li>
                <li>Platform: Horizon World (VR/PC/Mobile).</li>
            </ul>
        </div>
    
        <div class="paragraph">
            <div class="notice">
            Available on <a href="https://horizon.meta.com/world/10162530732029711/?locale=en_US" target="_blank">Horizon World</a>.
            </div>
        </div>
    
        <div class="paragraph center">
        </div>`, "#461cd4", false, false),
    new ProjectData("project-8", "The Imitation", "imitation/imagenPrincipal.png", `
        <div class="paragraph">
            <strong>The Imitation</strong> First-person survival horror.
        </div>
        <div class="paragraph">
            Night guard in a museum. Creatures imitate exhibits. Hand-cranked flashlight as defense.
        </div>
        <div class="paragraph center">
            <iframe src="https://www.youtube.com/embed/LE3EplcP6UE" class="youtube" frameborder="0" allowfullscreen></iframe>
        </div>
        <div class="paragraph">
            <ul>
                <li>Engine: Unity Engine.</li>
                <li>Language: C#.</li>
                <li>Platform: Windows/VR.</li>
                <li>Inputs: WASD move, Mouse camera, E interact, SPACE reel flashlight.</li>
            </ul>
        </div>
        <div class="paragraph">
            <div class="notice">
            Available on <a href="https://juanifa.itch.io/the-imitation" target="_blank">itch.io</a>.
            GGJ entry on <a href="https://globalgamejam.org/games/2026/imitation-5" target="_blank">Global Game Jam</a>.
            </div>
        </div>
        <div class="paragraph center">
            <img class="pc-screenshot" src="imitation/VU53NH.png" alt="The Imitation screenshot 1" />
            <img class="pc-screenshot" src="imitation/rqhQu4.png" alt="The Imitation screenshot 2" />
            <img class="pc-screenshot" src="imitation/oDsLAx.png" alt="The Imitation screenshot 3" />
        </div>
        `, "#4C0D02"),
];